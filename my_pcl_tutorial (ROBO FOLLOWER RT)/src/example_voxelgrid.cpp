...
cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
  ...
  pcl_conversions::toPCL(*cloud_msg, *cloud); //"toPCL" muct copy entire point cloud because its input is a "const"

  // Perform the actual filtering
  pcl::VoxelGrid<pcl::PCLPointCloud2> sor;
  sor.setInputCloud (cloudPtr); // vs. "sor.setInputCloud (cloud);" where "cloud" was a "pcl::PCLPointCloud2::Ptr" datatype
  sor.setLeafSize (0.1, 0.1, 0.1);
  sor.filter (cloud_filtered); // vs. sor.filter (*cloud_filtered); where "cloud_filtered" was a "pcl::PCLPointCloud2" datatype
